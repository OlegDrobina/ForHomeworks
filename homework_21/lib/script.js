...
function generateTaskListRecord(taskId, taskCaption) {
  let isCompleted = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
  let shouldSaveInStorage = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : true;
...
function saveTaskToLocalStorageOnTaskCreation(id, caption) {
  ...
  localStorage.setItem("listOfTasks", storageTasks ? JSON.stringify([...JSON.parse(storageTasks), {
    id: id,
    caption: caption,
    isCompleted: false
  }]) : JSON.stringify([{
...
function saveTaskStatusInLocalStorage(evtTarget, listEl) {
  ...
  const taskFromLocalStorageIdx = JSON.parse(localStorageTasks).findIndex(task => task.id == taskId);
...
function removeTaskFromStorage(event) {
  ...
  const localStorageTasksWithoutRemoved = localStorageTasks.filter(task => task.id !== taskId);
  localStorage.setItem("listOfTasks", JSON.stringify(localStorageTasksWithoutRemoved));
}