...
function* startProcessFetchResponseSaga(action) {
  try {
    const reqUrl = `${API.URL_SWAPI}${action.payload}`;
    const swapiResponse = yield call(fetchHelper, reqUrl);
    yield put(
      processFetchResponse(JSON.stringify(swapiResponse.result, false, 4))
    );
  } catch (e) {
    console.log(e);
  }
...
function* startProcessClearResponseSaga(action) {
...
function* watchStartProcessFetchResponse() {
  yield takeEvery(
    startProcessFetchResponse.type,
    startProcessFetchResponseSaga
  );
}

function* watchStartProcessClearResponse() {
  yield takeEvery(
    startProcessClearResponse.type,
    startProcessClearResponseSaga
  );
}
...
export default function* rootSaga() {
  yield all([
    ...
    watchModifyItem(),
    watchClearItems(),
    watchStartProcessFetchResponse(),
    watchStartProcessClearResponse(),
  ]);
}
